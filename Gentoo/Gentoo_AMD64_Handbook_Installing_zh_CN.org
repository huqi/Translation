...
用户可以选择为个人电脑命名。这看上去很简单，但是很多用户难以为他们的Linux计算机起一个合适的名字。为了加快速度，你需要知道这个设置不是最终的——后面还可以修改。下面的例子，主机名为 /tux/ 域名为 /homenetwork/ 。
...
第二，如果需要域名，请设置/etc/conf.d/net。这只有在ISP或者网络管理员需要时设置，或者有一个DNS服务器并且没有DHCP服务器。如果系统使用DHCP获取动态IP地址，不需要担心DNS或者域名。

#+BEGIN_QUOTE
*Note*

默认情况下系统没有/etc/conf.d/net文件，所以需要建一个。
#+END_QUOTE
...
如果没有设置域名，当用户登录系统时，会看到This is hostname.(none)的信息提示。这需要编辑/etc/issue并删除 =.\O= 行来修复这个问题。
#+END_QUOTE

如果需要一个NIS域名（用户如果不知道这是什么，那就不需要），定义如下：

#+BEGIN_SRC
root # nano -w /etc/conf.d/net
# 设置nis_domain_lo为NIS域名
...
请用 =bzless= 阅读/usr/share/doc/netifrc-*/net.example.bz2来获取更多设置DNS和NIS的信息。同时，你可以有兴趣安装[[http://packages.gentoo.org/package/net-dns/openresolv][net-dns/openresolv]]来管理DNS/NIS设置。
#+END_QUOTE

** 配置网络

在Gentoo Linux安装过程中，网络已经配置好了。然而，这只是在安装CD下，而不是已安装的环境。现在，配置已安装Gentoo Linux系统中的网络。

#+BEGIN_QUOTE
*备注*

更多的关于网络的信息，包括像绑定、桥接、802.1Q虚拟网或者无线网络设置等高级话题，可以查看Gentoo网络配置章节。
#+END_QUOTE

所有的网络信息集中在/etc/conf.d/net。它使用了一个简单的但也许并不直观语法。但是不要害怕，下面会有详细的说明。在/usr/share/doc/netifrc-*/net.example.bz2里有一个不同配置的完整注释。

首先安装[[http://packages.gentoo.org/package/net-misc/netifrc][net-misc/netifrc]]：

#+BEGIN_SRC
root # emerge --ask --noreplace net-misc/netifrc
#+END_SRC

默认使用DHCP。为了能使DHCP工作，需要安装一个DHCP客户端。在下面安装系统必要工具时会说明。

如果因为DHCP需要明确的选项，因为不总是需要DHCP，所以网络连接需要配置，那么打开/etc/conf.d/net：

#+BEGIN_SRC
root # nano -w /etc/conf.d/net
#+END_SRC

设置 =config_eth0= 和 =route_eth0= 为IP地址和路由信息：

#+BEGIN_QUOTE
*备注*

这里假设网络接口是eth0。这依赖于系统。如果安装媒介正确连网的话，这里建议使用安装媒介使用的网络接口名称，可以在网络接口命名找到更多信息。
#+END_QUOTE

#+BEGIN_SRC
文件 /etc/conf.d/net静态IP设置

config_eth0="192.168.0.2 netmask 255.255.255.0 brd 192.168.0.255"
routes_eth0="default via 192.168.0.1"
#+END_SRC

使用DHCP，设置 =config_eth0= ：

#+BEGIN_SRC
文件 /etc/conf.d/net DHCP设置

config_eth0="dhcp"
#+END_SRC

请阅读/usr/share/doc/netifrc-*/net.example.bz2获得所有有效选项。如果需要详细设置DHCP选项，请仔细阅读DHCP客户端帮助页面。

如果系统有好几个网络接口，那么重复上面 =config_eth1= 和 =config_eth2= 等步骤。

现在保存退出并继续。

** 开机启动网络

要想开机激活网络接口，需要将它加入到默认运行级别。
...
如果系统有几个网络接口，需要把像上面net.eth0那样创建相应的net.*文件。

在系统启动后，假如我们发现网络接口名（当前为 =eth0= ）错了，那么执行下面的步骤来纠正：

  1. 更新/etc/conf.d/net文件为正确的接口名（比如 =enp3s0= 替换 =eth0= ）
  2. 创建新的符号链接（如/etc/init.d/net.enp3s0）
  3. 删除旧的符号链接（ =rm /etc/init.d/net.eth0= ）
  4. 添加新的接口到默认运行级
  5. 删除旧的接口， =rc-update del net.eth0 default=

** 主机文件

下面了解一下Linux网络环境。这被定义在/etc/hosts文件中，这个文件用来当域名服务不能正确解析时，来解决域名和IP之间的转换。
...
PCMCIA用户需要安装[[http://packages.gentoo.org/package/sys-apps/pcmciautils][sys-apps/pcmciautils]]包。

* 系统信息

** Root密码

使用 =passwd= 命令设置root密码。

#+BEGIN_SRC
root # passwd
#+END_SRC

Linux的root用户是一个拥有所有权限的账户，所以需要选择一个强密码。一会儿会建立一个用于日常工作的常规用户。

** 初始化启动配置

Gentoo（使用OpenRC的近期版本）使用/etc/rc.conf来配置服务，开机启动，和关闭系统。打开/etc/rc.conf研究所有的注释。查看设置并且按照你的需要修改。

#+BEGIN_SRC
root # nano -w /etc/rc.conf
#+END_SRC

下一步，打开/etc/conf.d/keymaps来配置键盘。编辑并配置一个正确的键盘。

#+BEGIN_SRC
root # nano -w /etc/conf.d/keymaps
#+END_SRC

要特别小心键盘变量。如果配置了一个不正确的键盘，在输入时会出现奇怪的结果。

最后，编辑/etc/conf.d/hwclock来设置时钟。你可以根据自已的偏好来编辑它。

#+BEGIN_SRC
root # nano -w /etc/conf.d/hwclock
#+END_SRC

如果硬件没有使用UTC时钟，那么需要在文件中设置clock="local"。不然的话系统会显示错误的时间。

* 系统日志

某些工具stage3没有提供，是因为一些包提供了相同的功能。现在是用户选择安装的时候了。

首先给系统安装一个日志工具。Unix和Linux有非常好的日志能力——如果需要，系统会把所有事情都记录到日志文件中。这些都要通过系统日志工具完成。

Gentoo提供了几种系统日志工具。例如如下：

  - [[http://packages.gentoo.org/package/app-admin/sysklogd][app-admin/sysklogd]]，传统的系统日志服务
  - [[http:http://packages.gentoo.org/package/app-admin/syslog-ng][app-admin/syslog-ng]]，高级的系统日志管理器
  - [[http://packages.gentoo.org/package/app-admin/metalog][app-admin/metalog]]，高度定制化的日志管理器

可以通过portage安装其它可用的日志服务——每天都会增加可用的包。

如果使用sysklogd或者syslog-ng，建议安装[[http://packages.gentoo.org/package/app-admin/logrotate][app-admin/logrotage]]作为系统日志管理器，并且不要使用任何旋转日志功能。

安装选择的系统日志管理器，emerge它并用 =rc-update= 把它加入到默认运行级别当中。下面是安装[[http://packages.gentoo.org/package/app-admin/syslog-ng][app-admin/syslog-ng]]。
...
下一个是cron守护进程。虽然它是可选安装，并不是每个系统必须的工具，但是安装它是明智的选择。

Cron守护进程是执行计划的命令。如果需要定时（例如日常，星期或者月份）执行一些命令，这是很好用的。

Gentoo提供了几种cron守护进程，[[http://packages.gentoo.org/package/sys-process/bcron][sys-process/bcron]]，[[http://packages.gentoo.org/package/sys-process/dcron][sys-process/dcron]]，[[http://packages.gentoo.org/package/sys-process/fcron][sys-process/fcron]]和[[http://packages.gentoo.org/package/sys-process/cronie][sys-process/cronie。像安装系统日志管理器一样安装其中的一个。下面的例子使用]][[http://packages.gentoo.org/package/sys-process/cronie][sys-process/cronie]]：
...
如果想索引文件系统，使系统拥有更快的文件定位能力，安装[[http://packages.gentoo.org/package/sys-apps/mlocate][sys-apps/mlocate]]。
...
安装完成后，如果想行程访问系统，需要把sshd初始化脚本加入到默认启动级别：

#+BEGIN_SRC
root # rc-update add sshd default
#+END_SRC

如果需要串口控制台（可能是远程服务器），去掉/etc/inittab串口控制台部分的注释。
...
根据使用的文件系统，必须安装相应的工具（检查文件系统完整性，创建附加的文件系统等）。请注意ext2，ext3或者ext4文件系统（e2fsprogs）已经做为系统的一步分安装了。
...
如果不需要安装额外的网络工具，请跳到[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Bootloader][配置引导程序]]章节继续阅读。

** 安装DHCP客户端
#+BGEIN_QUOTE
*重要*

通过选项，多数的用户可能发现他们在网络中需要DHCP客户端。请安装DHCP客户端。如果忘记了这个步骤，那么系统可能不能连接到网络，接下来也不能正常的安装DHCP。
#+END_QUOTE

想要为一个或者多个网络接口自动获取IP地址，必须安装一个DHCP客户端。我们推荐使用[[http://packages.gentoo.org/package/net-misc/dhcpcd][net-misc/dhcpcd]]，然而portage树中有很多DHCP客户端也可以使用：
...
如果使用PPP连接网络，请安装[[http://packages.gentoo.org/package/net-dialup/ppp][net-dialup/ppp]]包：

#+BEGIN_SRC
root # emerge --ask net-dialup/ppp
#+END_SRC

现在请继续阅读[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Bootloader][配置引导程序]]。

* 选择引导程序

在配置完Linux内核后，安装了系统工具并编辑配置文件，现在安装最后一个Linux重要部分：引导程序。

引导程序用于启动Linux内核——如果没有安装它，在按动开机按钮后，系统将不知道怎么进一步处理。

对于amd64位，我们说明了怎么在基于BIOS系统中配置[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Full/Installation#Default:_Using_GRUB2][GRUB2]]或者[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Full/Installation#Alternative:_Using_LILO][LILO]]。基于UEFI系统配置[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Full/Installation#Alternative:_Using_efibootmgr][efibootmgr]]。

* 默认：使用GRUB2

以前，Gentoo Linux使用传统的GRUB做为建议的引导程序，就像他的名字一样，旧的GRUB包已经不再被积极维护，而已经被更强大的GRUB2替代。要想获得更多关于传统[[https://wiki.gentoo.org/wiki/GRUB][GRUB]]的信息，请查看Gentoo Wiki上GRUB相关文档。

** 安装GRUB2

[[http://packages.gentoo.org/package/sys-boot/grub][sys-boot/grub]]包提供了GRUB2。

#+BEGIN_SRC
root # emerge --ask sys-boot/grub
#+END_SRC

现在已经在系统上安装了GRUB2，但是还没有激活。

** 配置GRUB2

下一步，安装GRUB2必要的文件到 ~/boot/grub/~ 。假设第一个磁盘分区（系统的启动分区）是 ~/dev/sda~ ，可以使用如下命令操作：

#+BEGIN_SRC
root # grub2-install /dev/sda
#+END_SRC

下一步，我们可以根据用户配置的 ~/etc/default/grub~ 文件和 ~/etc/grub.d~ 文件来生成GRUB2的配置。大多数情况下，不需要用户配置，GRUB2会自动检测启动内核（在 =/boot/= 中最高版本）和根文件系统。

使用 =grub2-mkconfig= 生成最终的GRUB2配置文件：
...
命令的输出结果一定会提及，至少发现了一个需要启动的系统的Linux镜像。如果使用了initramfs或者用 =genkernel= 来构建内核，那么一般情况会正常检测到下确的initrd镜像。如果不是这样构建系统，进入 ~/boot/~ 目录使用 =ls= 命令列出目录内容。如果镜像文件确实不存在，请回看内核配置和相关的安装指令。

* 可选：使用LILO

** 安装LILO

LILO，命名LInuxLOader，是一个可靠的Linux启动引导程序。但是，他年少一些GRUB的特性。有一些系统不支持GRUB启动，这是目前还使用LILO的原因。当然，也因为一些用户已经知道并习惯使用它。不管怎么样，Gentoo两个都支持。

安装LILO很简单；只要使用emerge。
...
要想配置LILO，首先创建/etc/lilo.conf文件：

#+BEGIN_SRC
root # nano -w /etc/lilo.conf
#+END_SRC

在配置文件中，各节用来指定可引导内核。请确认已经知道内核文件（带版本号）和initramfs文件，他们需要在配置文件中指定。

#+BEGIN_SRC
*注释*

如果主文件系统使用的是JFS，需要在每个启动条目后加上一行append="ro"，这样可以让它在可读写挂在前记录日志。

#+BEGIN_SRC
文件 /etc/lilo.conf LILO配置示例

boot=/dev/sda             # 在MBR系统上安装LILO
prompt                    # 用户可以选择其它选项
timeout=50                # 在启动默认选项时等待5秒
default=gentoo            # 超时后启动“Gentoo”选项

image=/boot/vmlinuz-3.16.5-gentoo
  label=gentoo            # 名称
  read-only               # 启动时根分区只读，不要修改！
  root=/dev/sda4          # 根文件系统的位置

image=/boot/vmlinuz-3.16.5-gentoo
  label=gentoo.rescue     # 名称
  read-only               # 启动时根分区只读，不要修改！
  root=/dev/sda4          # 根文件系统的位置
  append="init=/bin/bb"   # 启动Gentoo静态急救

# 下面两行只针对Windows双启动
# 示例，Windows在/dev/sda6中
other=/dev/sda6
  label=windows
#+END_SRC
...
如果内核镜像使用了不同的分区方案，请根据实际情况修改。
#+END_QUOTE

如果需要initramfs，那么需要在配置文件中指定initramfs文件位置，通知initramfs真正的根设置位置：

#+BEGIN_SRC
文件 /etc/lilo.conf 在启动项中增加initramfs信息
...
如果需要在内核中增加选项，需要 =append= 语句。例如，增加 =video= 语句来打开帧缓冲：

#+BEGIN_SRC
文件 /etc/lilo.conf 在启动选项中增加video参数
...
如果用户使用 =genkernel= 来构建系统，需要使用和安装CD一样的启动选项。例如，如果需要支持SCSI设备，在内核选项上增加 =doscsi= 。

现在保存并退出。

最后，执行 =/sbin/lilo= ，LILO会把配置部署到系统中（也就是说把自己安装到磁盘上）。需要住，系统构建安装新内核或者修改了lilo.conf文件后，都要执行一次 =/sbin/lilo= 。
...
基于UEFI的系统，UEFI固件可以自引导。这些系统不需要额外的启动引导程序，虽然已经有一些基于EFI并扩充功能了的启动引导程序。

[[http://packages.gentoo.org/package/sys-boot/efibootmgr][sys-boot/efibootmgr]]程序不是一个启动引导程序，只是一个UEFI固件的交互接口，可以更新设置，所以在Linux内核安装成功前，可以用附加选项来启动（如果需要）或者设置多个启动项。这些交互是通过EFI变量完成的（因此需要支持EFI变量）

#+BEGIN_QUOTE
*备注*

efibootmgr在UEFI系统上不是必须的。Linux可以直接自己启动，附加的启动项可以做为内核的一部分（内核配置允许用户指定启动参数）。甚至initramfs也可以作为内核的一部分。
#+END_QUOTE
...
使用EFI描述时，必须使用 =\= 作为目录分隔符。
#+END_QUOTE

修改完成后，重新启动，可以看到一个叫做“Gentoo”的启动选项。

* 重新启动系统

退出chrooted环境并卸载所有已挂在的分区。输入最后一个神奇的指令： =reboot= 。
...
当然，不要忘记拿出启动CD，否则会再次启动安装CD，而不是进入新的Gentoo系统。

启动新安装的Gentoo环境后，请阅读[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Finalizing][完成Gentoo安装]]。

* 用户管理

** 增加普通用户

用户root用户来操作Unix/Linux系统是非常危险的，我们应该尽量避免使用。所以强烈建议增加一个日常使用的账户。

群组定义了用户可以使用的功能。下面的表格列出了一些重要群组：
...
| floppy  | 可以使用软驱设备               |
| games   | 可以玩游戏                     |
| portage | 可以portage有限的资源          |
| usb     | 可以使用USB设备                |
| video   | 可以使用视频抓取硬件和硬件加速 |
| wheel   | 可以使用su                     |

例如，创建一个属于wheel、users和audio群组的名为john的用户，首先用root（只有root可以创建用户）登录并且执行 =useradd= ：
...
如果用户需要root权限来操作某些任务，可以使用 =su -= 来临时获取root权限。另一种方法是使用[[https://wiki.gentoo.org/wiki/Sudo][sudo]]包，如果配置正确，使用起来很稳定。

* 磁盘清理

** 删除tarballs

Gentoo安装完成并且重新启动后，如果一切顺利，我们现在可以从磁盘中删除下载的stage3包。他被下载到了 =/= 目录下。
...
下一步做什么？怎么选择？先从哪里开始探索？Gentoo给用户提供了很多可能性，所以有很多特性文档（很少没有）。

绝对应该看一看Gentoo手册的下一部分[[https://wiki.gentoo.org/wiki/Handbook:AMD64/Working/Portage][用Gentoo工作]]，那里解释了怎么保持最新软件，怎么安装更多的软件，USE标志是什么，Gentoo怎么初始化系统的工作，等等。

除了手册，你应该鼓起勇气来探索Gentoo Wiki来查找社区提供的附加文档。Gentoo文档团队也提供了一个[[https://wiki.gentoo.org/wiki/Project:Documentation/Overview][文档概述]]，列出了精心挑选的Wiki文档。例如，有一些[[https://wiki.gentoo.org/wiki/Localization/HOWTO][本地化的文档]]使系统更有家的感觉。

** Gentoo在线

欢迎每个人来访问我们[[https://forums.gentoo.org/][Gentoo论坛]]或者我们的[[https://www.gentoo.org/main/en/irc.xml][Gentoo IRC频道]]。

我们也有几个向所有用户开放的[[https://www.gentoo.org/main/en/lists.xml][邮件列表]]，加入的方法也在那个页面当中。
