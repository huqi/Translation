...
Previously, Gentoo Linux used what is now called GRUB Legacy as the recommended boot loader. As the name implies, the older GRUB package is no longer actively maintained and has been superseded by GRUB2. For more information about the legacy [[https://wiki.gentoo.org/wiki/GRUB][GRUB]], please refer to its GRUB article on the Gentoo Wiki.
...
Once rebooted in the freshly installed Gentoo environment, finish up with [[https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Finalizing][Finalizing the Gentoo installation]].
...
If a user ever needs to perform some task as root, they can use =su -= to temporarily receive root privileges. Another way is to use the [[https://wiki.gentoo.org/wiki/Sudo][sudo]] package which is, if correctly configured, very secure.
...
Apart from the handbook, you should also feel encouraged to explore other corners of the Gentoo Wiki to find additional, community-provided documentation. The Gentoo documentation team also offers a [[https://wiki.gentoo.org/wiki/Project:Documentation/Overview][Documentation overview]] which lists a fine selection of Wiki articles. For instance, it refers to the [[https://wiki.gentoo.org/wiki/Localization/HOWTO][localization guide]] to make a system feel more at home.

** Gentoo online

Everyone is of course always welcome on our [[https://forums.gentoo.org/][Gentoo forums]] or on one of our many [[https://www.gentoo.org/main/en/irc.xml][Gentoo IRC channels]].

We also have [[https://www.gentoo.org/main/en/lists.xml][several mailing lists]] open to all our users. Information on how to join is contained in that page.
